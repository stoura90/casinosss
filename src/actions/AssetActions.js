      ...
      if (!inProgress[id]) {
        ...
        Apis.instance()
          ...
          .then((assets) => {
            ...
            let dynamicPromise = Apis.instance().db_api().exec('get_objects', [dynamicIDS])
    ...
    return (dispatch) => {
      if (!inProgress[id]) {
        ...
        if (Utils.is_object_id(id)) {
          assetPromise = Apis.instance()
            .db_api()
            .exec('get_objects', [[id]])
        } else {
          assetPromise = Apis.instance().db_api().exec('list_assets', [id, 1])
        }
        ...
        return assetPromise
          .then((asset) => {
            ...
            let bitAssetPromise = asset[0].bitasset_data_id
              ? Apis.instance()
                  .db_api()
                  .exec('get_objects', [[asset[0].bitasset_data_id]])
              : null

            Promise.all([
              Apis.instance()
                .db_api()
                .exec('get_objects', [[asset[0].dynamic_asset_data_id]]),
              bitAssetPromise,
            ]).then((results) => {
              delete inProgress[id]
              dispatch({
