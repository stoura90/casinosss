...
class MarketsActions {
    ...
    return (dispatch) => {
      ...
      if (!marketStats[market] || refresh) {
        Promise.all([
          ...
          Apis.instance()
            .history_api()
            .exec('get_fill_order_history', [base.get('id'), quote.get('id'), 1]),
        ]).then((result) => {
          marketStats[market] = {
  ...
  subscribeMarket(base, quote, bucketSize) {
    ...
    return (dispatch) => {
      let subscription = (subResult) => {
        ...
        if (!dispatchSubTimeout) {
          ...
          dispatchSubTimeout = setTimeout(() => {
            ...
            if (isMarketAsset) {
              callPromise = Apis.instance().db_api().exec('get_call_orders', [marketAsset.id, 100])
              settlePromise = Apis.instance()
                .db_api()
                .exec('get_settle_orders', [marketAsset.id, 100])
            }
            ...
            Promise.all([
              Apis.instance()
                .db_api()
                .exec('get_limit_orders', [base.get('id'), quote.get('id'), 100]),
              ...
              !hasFill
                ...
                : Apis.instance()
                    .history_api()
                    .exec('get_fill_order_history', [base.get('id'), quote.get('id'), 200]),
              !hasFill
                ? null
                : Apis.instance()
                    .history_api()
      ...
      if (!subs[subID] || currentBucketSize !== bucketSize) {
        ...
        if (isMarketAsset) {
          callPromise = Apis.instance().db_api().exec('get_call_orders', [marketAsset.id, 100])
          settlePromise = Apis.instance().db_api().exec('get_settle_orders', [marketAsset.id, 100])
        }
        ...
        return Promise.all([
          Apis.instance()
            .db_api()
            .exec('subscribe_to_market', [subscription, base.get('id'), quote.get('id')]),
          Apis.instance()
            .db_api()
            .exec('get_limit_orders', [base.get('id'), quote.get('id'), 100]),
          callPromise,
  ...
  createLimitOrder(
    ...
    if (
      feeAsset.getIn(['options', 'core_exchange_rate', 'base', 'asset_id']) === '1.3.0' &&
      feeAsset.getIn(['options', 'core_exchange_rate', 'quote', 'asset_id']) === '1.3.0'
    ) {
      fee_asset_id = '1.3.0'
    }
