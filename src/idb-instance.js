...
var iDB = (function () {
  ...
  return {
    ...
    init_instance: function (indexedDBimpl, chain_id = Apis.instance().chain_id) {
      if (!_instance) {
        if (indexedDBimpl) {
    ...
    setCachedProperty: function (name, value) {
      ...
      return idb_helper.on_request_end(store.put({ name, value })).catch((error) => {
        console.error(error)
        throw error
      })
    },
      ...
      return Promise.all(promises).then((results) => {
        ...
        for (let i = 0; i < store_names.length; i++) {
          ...
          if (store_name === 'wallet') {
            ...
            for (let wallet of wallet_array) wallet.backup_date = new Date().toISOString()
          }
