...
class BuySell extends React.Component {
  ...
  render() {
    ...
    let balanceAmount = balance
      ? utils.get_asset_amount(balance.get('balance'), { precision: balancePrecision })
      : 0
    if (!balanceAmount) {
      balanceAmount = 0
    }
    ...
    let hasBalance =
      type === 'bid' ? balanceAmount >= parseFloat(total) : balanceAmount >= parseFloat(amount)
    ...
    if (
      feeAssets[1] &&
      feeAssets[1].getIn(['options', 'core_exchange_rate', 'quote', 'asset_id']) === '1.3.0' &&
      feeAssets[1].getIn(['options', 'core_exchange_rate', 'base', 'asset_id']) === '1.3.0'
    ) {
      ...
      <div className={this.props.className}>
        <div className="exchange-bordered buy-sell-container">
          <div className={'exchange-content-header ' + type}>
            <span>
              {buttonText} <AssetName name={quote.get('symbol')} />
            </span>
            {this.props.onFlip ? (
              <span onClick={this.props.onFlip} style={{ cursor: 'pointer', fontSize: '1rem' }}>
                {' '}
                &#8646;
              </span>
            ) : null}
          ...
          <form className={(!this.props.isOpen ? 'hide-container ' : '') + 'order-form'} noValidate>
            <div className="grid-block vertical no-overflow no-padding">
              ...
              <div className="grid-block no-padding buy-sell-row">
                ...
                <div
                  className="grid-block small-3 no-margin no-overflow buy-sell-box"
                  ...
                  <tbody>
                    <tr className="buy-sell-info">
                      ...
                      <td style={{ paddingLeft: 5, textAlign: 'right' }}>
                        ...
                        >
                          {utils.format_number(balanceAmount, balancePrecision)}{' '}
