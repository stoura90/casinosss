...
;(function (HC) {
  ...
  HC.wrap(HC.Series.prototype, 'setVisible', function (proceed, vis, redraw) {
    var newVis =
        vis === UNDEFINED ? (this.isVisible === UNDEFINED ? !this.visible : !this.isVisible) : vis,
            ...
            if (ind.options.tooltip) {
              ...
              pointFormat += HC.format(indPointFormat, {
                point: {
                  ...
                  color:
                    indLen > 3
                      ? ind.graph[1].element.attributes.stroke.value
                      : ind.graph[0].element.attributes.stroke.value,
  ...
  HC.wrap(HC.Legend.prototype, 'renderItem', function (p, item) {
    if (item instanceof window.Indicator) {
  ...
  HC.wrap(
    HC.Legend.prototype,
    'setItemEvents',
    function (p, item, legendItem, useHTML, itemStyle, itemHiddenStyle) {
  ...
  window.Indicator.prototype = {
    /*
     * Initialize the indicator
     */
    init: function (chart, options) {
      // set default params, when not specified in params
      if (!window.Indicator.prototype[options.type])
        error('Indicator: ' + options.type + ' not found!')
      options.params = merge(
        {},
        window.Indicator.prototype[options.type].getDefaultOptions(),
        options.params,
      )
    ...
    render: function (redraw) {
      ...
      if (!series) {
        ...
        arrayValues = window.Indicator.prototype[options.type].getValues(
          chart,
          { points: [] },
          options,
        ...
        this.graph = graph = window.Indicator.prototype[options.type].getGraph(
          chart,
          series,
          options,
          this.values,
    ...
    drawGraph: function () {
      ...
      ind.graph = window.Indicator.prototype[ind.options.type].getGraph(
        ind.chart,
        ind.series,
        ind.options,
        ind.values,
    ...
    groupPoints: function (series) {
      ...
      if (series.currentDataGrouping) {
        ...
        groupIntervalFactor =
          (xAxis.options.ordinal && xAxis.getGroupIntervalFactor(xMin, xMax, series)) || 1
        interval =
          ((series.groupPixelWidth * (xMax - xMin)) / series.chart.plotSizeX) * groupIntervalFactor
    ...
    groupData: function (xData, yData, groupPositions, approximation) {
      var ind = this,
        ...
        values = [[], [], [], []],
        approximationFn =
          typeof approximation === 'function' ? approximation : HC.approximations[approximation],
        i
      ...
      for (; i <= dataLength; i++) {
        ...
        while (
          (groupPositions[1] !== UNDEFINED && xData[i] >= groupPositions[1]) ||
          i === dataLength
        ) {
          // get the last group
    ...
    drawLegendSymbol: function (legend) {
      var options = this.options,
        ...
        verticalCenter =
          legend.baseline -
          Math.round(
            renderer.fontMetrics(legendOptions.itemStyle.fontSize, this.legendItem).b * 0.3,
          ),
        attr
  ...
  extend(Chart.prototype, {
    ...
    addIndicator: function (options, redraw) {
      ...
      item = new window.Indicator(chart, options)
  ...
  Chart.prototype.callbacks.push(function (chart) {
    ...
    if (exportingFlag && optionsLen > 0 && chart.series && chart.series.length > 0) {
      // #16 & #27
