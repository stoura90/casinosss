...
;(function (H) {
  ...
  function renderCurrentPriceIndicator(chart) {
    if (
      !chart.series[1] ||
      (chart.series[1] && chart.series[1].yData && !chart.series[1].yData.length)
    ) {
      return
    }
    ...
    if (options.enabled) {
      ...
      if (!isRendered) {
        ...
        // label
        label = renderer
          .text(utils.format_number(currentPrice, options.precision), x, y)
          .attr({
            zIndex: 2,
          })
          .css({
        ...
        // box
        box = renderer
          .rect(x, y - height / 2, options.width ? options.width : width, height)
          .attr({
