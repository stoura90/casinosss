...
class PrivateKeyStore extends BaseStore {
  ...
  onAddKey({ private_key_object, transaction, resolve }) {
    ...
    let p = new Promise((resolve, reject) => {
      PrivateKeyTcomb(private_key_object)
      let duplicate = false
      let p = idb_helper.add(transaction.objectStore('private_keys'), private_key_object)
      ...
      p.catch((event) => {
        ...
        if (private_key_object.brainkey_sequence == null) this.binaryBackupRecommended() // non-deterministic
    ...
    if (private_key) {
      ...
      try {
        memo_text = private_key
          ? Aes.decrypt_with_checksum(private_key, public_key, memo.nonce, memo.message).toString(
              'utf-8',
            )
          : null
