...
class MarketsStore {
  ...
  _getFeed() {
    ...
    let settlePrice = this[this.invertedCalls ? 'baseAsset' : 'quoteAsset'].getIn([
      'bitasset',
      'current_feed',
      'settlement_price',
    ])
    ...
    try {
      ...
      this.is_prediction_market = this[this.invertedCalls ? 'baseAsset' : 'quoteAsset'].getIn(
        ['bitasset', 'is_prediction_market'],
        false,
      )
      this.bitasset_options = this[this.invertedCalls ? 'baseAsset' : 'quoteAsset']
        .getIn(['bitasset', 'options'])
        .toJS()
      /* Prediction markets don't need feeds for shorting, so the settlement price can be set to 1:1 */
      if (
        this.is_prediction_market &&
        settlePrice.getIn(['base', 'asset_id']) === settlePrice.getIn(['quote', 'asset_id'])
      ) {
        const backingAsset = this.bitasset_options.short_backing_asset
        if (!assets[backingAsset])
          assets[backingAsset] = { precision: this.quoteAsset.get('precision') }
  ...
  _combineOrders() {
    ...
    this.marketData.lowestAsk = !this.marketData.combinedAsks.length
      ? nullPrice
      : this.marketData.combinedAsks[0]

    this.marketData.highestBid = !this.marketData.combinedBids.length
      ? nullPrice
