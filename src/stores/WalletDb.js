...
class WalletDb extends BaseStore {
  ...
  checkNextGeneratedKey() {
    ...
    if (this.chainstore_account_ids_by_key === ChainStore.account_ids_by_key) return // no change
  ...
  onCreateWallet(password_plaintext, brainkey_plaintext, unlock = false, public_name = 'default') {
    let walletCreateFct = (dictionary) => {
      return new Promise((resolve, reject) => {
        ...
        if (!brainkey_plaintext) brainkey_plaintext = key.suggest_brain_key(dictionary.en)
    ...
    if (window.electron) {
      ...
      return Promise.all([dictionaryPromise])
        .then((res) => {
          return brainkey_plaintext ? walletCreateFct(null) : res[0].json().then(walletCreateFct)
        })
        .catch((err) => {
  ...
  changePassword(old_password, new_password, unlock = false) {
    return new Promise((resolve) => {
      let wallet = this.state.wallet
      if (!this.validatePassword(old_password)) throw new Error('wrong password')
  ...
  importKeysWorker(private_key_objs) {
    return new Promise((resolve, reject) => {
      let pubkeys = []
      for (let private_key_obj of private_key_objs) pubkeys.push(private_key_obj.public_key_string)
      ...
      let AesWorker = require('../workers/AesWorker')
      ...
      worker.onmessage = (event) => {
        try {
          ...
          try {
            ...
            if (private_key_objs.length != duplicate_count) _this.setWalletModified(transaction)
  ...
  saveKey(
    ...
    let p1 = PrivateKeyActions.addKey(private_key_object, transaction).then((ret) => {
