...
class Transfer extends React.Component {
  ...
  _setTotal(asset_id, balance_id, fee, fee_asset_id, e) {
    ...
    if (balanceObject) {
      let amount = (
        utils.get_asset_amount(balanceObject.get('balance'), transferAsset) -
        (asset_id === fee_asset_id ? fee : 0)
      ).toString()
    ...
    if (from_account && from_account.get('balances') && !from_error) {
      ...
      if (feeAsset && feeAsset.get('id') !== '1.3.0' && core) {
        let price = utils.convertPrice(
          core,
          feeAsset.getIn(['options', 'core_exchange_rate']).toJS(),
          null,
          feeAsset.get('id'),
        )
      ...
      if (core) {
        fee = utils.limitByPrecision(
          utils.get_asset_amount(fee, feeAsset || core),
          feeAsset ? feeAsset.get('precision') : core.get('precision'),
        )
      }
      ...
      <div className="grid-block vertical">
        ...
        >
          <form
            ...
            className="grid-content medium-6 large-4 large-offset-2 full-width-content"
          ...
          >
            ...
            <div className="content-block transfer-input">
              <AmountSelector
                ...
                asset={
                  fee_asset_types.length && feeAsset
                    ? feeAsset.get('id')
          ...
          <div className="grid-content medium-6 large-4"></div>
        </div>
      </div>
    )
  }
