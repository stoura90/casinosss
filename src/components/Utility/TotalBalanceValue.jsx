...
class TotalValue extends React.Component {
  ...
  render() {
    // eslint-disable-next-line react/prop-types
    let { fromAssets, toAsset, balances, marketStats, collateral, debt, openOrders, inHeader } =
      this.props
    let coreAsset = ChainStore.getAsset('1.3.0')
    ...
    balances.forEach((balance) => {
      if (balance.asset_id === toAsset.get('id')) {
        ...
        if (fromAsset) {
          let eqValue = this._convertValue(
            balance.amount,
            fromAsset,
            toAsset,
            marketStats,
      ...
      return (
        <div className="tooltip" data-tip={totalsTip} data-place="bottom" data-html={true}>
          <FormattedAsset
            ...
            asset={toAsset.get('id')}
            decimalOffset={
              toAsset.get('symbol').indexOf('BTC') === -1 ? toAsset.get('precision') : 4
            }
          />
        </div>
      )
