...
class ValueComponent extends React.Component {
    ...
    if (coreAsset) {
      if (this.props.fromAsset.get('id') !== coreAsset.get('id')) {
        ...
        this.fromStatsInterval = setInterval(
          MarketsActions.getMarketStats.bind(this, coreAsset, this.props.fromAsset),
          5 * 60 * 1000,
        )
      }
  ...
  getValue() {
    ...
    let price = utils.convertPrice(
      fromStats && fromStats.close ? fromStats.close : fromAsset,
      toStats && toStats.close ? toStats.close : toAsset,
      fromID,
      toID,
    )
  ...
  render() {
    ...
    return (
      <FormattedAsset
        ...
        decimalOffset={
          // eslint-disable-next-line react/prop-types
          toSymbol.indexOf('BTC') !== -1 ? 4 : this.props.noDecimals ? toAsset.get('precision') : 0
        }
      />
    )
  }
