...
class CollateralPosition extends React.Component {
  ...
  _getMR() {
    return (
      this.props.debtAsset.getIn(['bitasset', 'current_feed', 'maintenance_collateral_ratio']) /
      1000
    )
  }
...
class CollateralPositionWrapper extends React.Component {
  ...
  render() {
    ...
    return (
      <CollateralPosition debtAsset={debtAsset} collateralAsset={collateralAsset} {...this.props} />
    )
  }
}
    ...
    <table className="table">
      <thead>
        <tr>
          ...
          <th className="column-hide-medium">
      ...
      <tbody>
        {/* eslint-disable-next-line react/prop-types */}
        {callOrders.map((id) => (
          <CollateralPositionWrapper key={id} object={id} account={account} />
        ))}
      </tbody>
    </table>
  )
