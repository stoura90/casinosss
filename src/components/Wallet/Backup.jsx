...
class BackupVerify extends Component {
  render() {
    return (
      <div style={{ maxWidth: '40rem' }}>
        <h3>
          <Translate content="wallet.verify_prior_backup" />
        </h3>
        ...
        <Upload>
          ...
          <DecryptBackup saveWalletObject={true}>
            <h4>
              <Translate content="wallet.verified" />
            </h4>
          </DecryptBackup>
          <Reset />
        </Upload>
    ...
    return (
      <div>
        ...
        {new FileReader().readAsBinaryString ? null : (
          <p className="error">
            Warning! You browser doesnot support some some file operations required to restore
    ...
    if (has_new_wallet)
      return (
        <span>
          <h5>
            {/* eslint-disable-next-line react/prop-types */}
            <Translate content="wallet.restore_success" name={new_wallet.toUpperCase()} />
          </h5>
          {/* eslint-disable-next-line react/prop-types */}
...
class Download extends Component {
  ...
  componentDidMount() {
    if (!this.isFileSaverSupported) notify.error('File saving is not supported')
  }
...
class Create extends Component {
  getBackupName() {
    ...
    let stampedName = `${name}_${date.getFullYear()}${month >= 10 ? month : '0' + month}${
      day >= 10 ? day : '0' + day
    }`
...
class LastBackupDate extends Component {
  render() {
    ...
    let backup_time = backup_date ? (
      <h4>
        <Translate content="wallet.last_backup" /> <FormattedDate value={backup_date} />
      </h4>
    ) : (
      <Translate
        style={{ paddingTop: 20 }}
  ...
  render() {
    return (
      <span>
        ...
        {this.props.backup.last_modified ? <div>{this.props.backup.last_modified}</div> : null}
        <br />
      </span>
    )
  }
  ...
  render() {
    ...
    return (
      <form onSubmit={this.onPassword.bind(this)}>
        ...
        />
        <Sha1 />
        <div type="submit" className="button outline" onClick={this.onPassword.bind(this)}>
  ...
  onPassword(e) {
    ...
    decryptWalletBackup(private_key.toWif(), contents)
      .then((wallet_object) => {
        ...
        if (this.props.saveWalletObject) BackupStore.setWalletObjct(wallet_object)
      })
      .catch((error) => {
        console.error('Error verifying wallet ' + this.props.backup.name, error, error.stack)
        if (error === 'invalid_decryption_key') notify.error('Invalid Password')
...
class Reset extends Component {
  ...
  render() {
    ...
    return (
      <span className="button cancel" onClick={this.onReset.bind(this)}>
        {label}
      </span>
    )
  }
