...
export default class BrainkeyInput extends Component {
    ...
    return (
      <span className="">
        <div>
          ...
          />
          <div style={{ textAlign: 'left' }} className="grid-content no-padding no-overflow">
            {checked_words}
          </div>
  ...
  formChange(event) {
    ...
    if (id === 'brnkey') {
      ...
      state.check_digits =
        brnkey.length < 50 ? null : hash.sha1(brnkey).toString('hex').substring(0, 4)
    }
    this.setState(state)
...
class MisspelledWord extends Component {
  render() {
    return (
      <span style={{ borderBottom: '1px dotted #ff0000', padding: '1px', margin: '1px' }}>
        <span style={{ borderBottom: '1px dotted #ff0000' }}>{this.props.children}</span>
      </span>
    )
  }
}
