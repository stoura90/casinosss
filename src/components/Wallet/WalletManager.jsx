          ...
          <ul>
            <li className="with-dropdown">
              {state.wallet_names.count() <= 1 ? (
                <div style={{ paddingLeft: 10 }}>{this.state.current_wallet}</div>
              ) : (
...
class WalletDelete extends Component {
  ...
  render() {
    ...
    let placeholder = (
      <option key="placeholder" value="" disabled={this.props.wallet_names.size > 1}></option>
    )
    ...
    return (
      <div style={{ paddingTop: 20 }}>
        ...
        >
          <Translate
            content={
              this.state.selected_wallet ? 'wallet.delete_wallet_name' : 'wallet.delete_wallet'
            }
