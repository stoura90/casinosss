    ...
    if (asset_types.length > 0) {
      ...
      if (current_asset_id) {
        ...
        if (account_balances[current_asset_id]) {
          this.balanceAmount =
            parseInt(ChainStore.getObject(account_balances[current_asset_id]).get('balance')) /
            Math.pow(10, parseInt(this.props.sellAsset.get('precision')))
        }
      } else {
        balance = 'No funds'
      ...
      <form className="grid-block vertical full-width-content">
        <div className="grid-container">
          <div className="content-block">
            <h3>
              <Translate
                content="gateway.transwiser.withdraw_title"
                asset={this.props.sellAsset.get('symbol')}
              />
            </h3>
          </div>
          <div className="content-block">
            <p>
              <Translate content="gateway.transwiser.withdraw_note" />
            </p>
          </div>
          <div className="content-block">
            <AmountSelector
            ...
            <div className="grid-block">
              ...
              {this.state.withdraw_amount_after_fee > 0 ? (
                <span>
                  {' '}
                  (
                  <Translate
                    content="gateway.transwiser.you_will_receive"
                    amount={this.state.withdraw_amount_after_fee}
                  />
                  )
                </span>
              ) : null}
          ...
          <div className="content-block full-width-content">
            <label>
              <Translate component="span" content="gateway.transwiser.alipay" />
            </label>
            <input
              type="text"
