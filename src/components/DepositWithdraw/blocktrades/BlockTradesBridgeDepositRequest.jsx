          ...
          (json) => {
            ...
            if (
              ...
              AccountActions.transfer(
                ...
                this.state.conversion_memo
                  ? new Buffer(this.state.conversion_memo, 'utf-8')
                  : this.state.conversion_memo,
                null,
                '1.3.0',
              )
...
class BlockTradesBridgeDepositRequest extends React.Component {
      ...
      .then((json_responses) => {
        ...
        if (allowed_deposit_coin_types.length) {
          ...
          if (
            ...
            output_coin_types_for_this_input.indexOf(this.props.initial_deposit_output_coin_type) !=
              -1
          )
        ...
        if (allowed_conversion_coin_types.length) {
          ...
          else conversion_output_coin_type = output_coin_types_for_this_input[0]
        }
        ...
        reply.json().then(
          (json) => {
            ...
            if (
              ...
              this.setState({
                input_address_and_memo: { address: json.inputAddress, memo: json.inputMemo },
              })
          },
          (error) => {
            delete this.state.input_address_requests_in_progress[input_coin_type][output_coin_type]
  ...
  getCachedOrFreshDepositLimit(deposit_withdraw_or_convert, input_coin_type, output_coin_type) {
    ...
    deposit_limit_promise.then(
      (reply) => {
        console.assert(
          reply.inputCoinType == input_coin_type && reply.outputCoinType == output_coin_type,
        ...
        if (
          this.state[deposit_withdraw_or_convert + '_input_coin_type'] == input_coin_type &&
    ...
    estimate_input_promise.then(
      (reply) => {
        ...
        if (
          ...
          this.setState({
            [deposit_withdraw_or_convert + '_estimated_input_amount']: reply.inputAmount,
  ...
  onInputCoinTypeChanged(deposit_withdraw_or_convert, event) {
    ...
    let possible_output_coin_types =
      this.state['allowed_mappings_for_' + deposit_withdraw_or_convert][new_input_coin_type]
      ...
      possible_output_coin_types.forEach((allowed_withdraw_output_coin_type) => {
        if (new_output_coin_type === allowed_withdraw_output_coin_type) {
          this.setState({
            ...
            supports_output_memos:
              this.state.coins_by_type[allowed_withdraw_output_coin_type].supportsOutputMemos,
          })
        }
      })
    }
    ...
    this.setState({
      ...
      [deposit_withdraw_or_convert + estimated_input_output_amount_state]:
        estimated_input_output_amount,
      [deposit_withdraw_or_convert + '_estimate_direction']:
        this.estimation_directions.output_from_input,
    })
  }
  ...
  onOutputCoinTypeChanged(deposit_withdraw_or_convert, event) {
    ...
    let withdraw_output_coin_types =
      this.state.allowed_mappings_for_withdraw[this.state.withdraw_input_coin_type]
  ...
  render() {
    if (!this.props.account || !this.props.issuer_account || !this.props.gateway) return <div></div>
    ...
    if (this.state.coin_info_request_state == this.coin_info_request_states.request_failed) {
      ...
      if (Object.getOwnPropertyNames(this.state.allowed_mappings_for_deposit).length > 0) {
        ...
        let deposit_output_coin_types =
          this.state.allowed_mappings_for_deposit[this.state.deposit_input_coin_type]
        ...
        if (this.state.deposit_error) deposit_error_element = <div>{this.state.deposit_error}</div>
      ...
      if (Object.getOwnPropertyNames(this.state.allowed_mappings_for_withdraw).length > 0) {
        ...
        if (this.state.withdraw_limit) {
          ...
          else withdraw_limit_element = <span>no limit</span>
        }
      ...
      if (Object.getOwnPropertyNames(this.state.allowed_mappings_for_conversion).length > 0) {
        ...
        Object.keys(this.state.allowed_mappings_for_conversion)
          ...
          .forEach((allowed_conversion_input_coin_type) => {
            conversion_input_coin_type_options.push(
              ...
              >
                {this.state.coins_by_type[allowed_conversion_input_coin_type].walletSymbol}
              </option>,
            )
          })
        ...
        let conversion_output_coin_types =
          this.state.allowed_mappings_for_conversion[this.state.conversion_input_coin_type]
        conversion_output_coin_types.forEach((allowed_conversion_output_coin_type) => {
          conversion_output_coin_type_options.push(
            ...
            >
              {this.state.coins_by_type[allowed_conversion_output_coin_type].symbol}
            </option>,
          )
        })
          ...
          if (this.state.conversion_limit.limit)
            conversion_limit_element = (
              <span className="deposit-limit">
                <Translate
                  content="gateway.limit"
                  amount={utils.format_number(this.state.conversion_limit.limit, 8)}
                  symbol={
                    this.state.coins_by_type[this.state.conversion_input_coin_type].walletSymbol
                  }
                />
              </span>
            )
          else conversion_limit_element = <span>no limit</span>
        }
