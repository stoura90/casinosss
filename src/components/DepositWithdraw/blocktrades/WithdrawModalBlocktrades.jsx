...
class WithdrawModalBlocktrades extends React.Component {
  ...
  constructor(props) {
    ...
    fetch(
      this.props.url +
        ...
        encodeURIComponent(
          localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`) !== null
            ? localStorage.getItem(`history_address_last_${this.props.output_wallet_type}`)
            : '',
        ),
      {
        method: 'get',
  ...
  onSelectChanged(index, e) {
    let new_withdraw_address = JSON.parse(
      localStorage.getItem(`history_address_${this.props.output_wallet_type}`),
    )[index]
    localStorage.setItem(
      `history_address_last_${this.props.output_wallet_type}`,
      JSON.parse(localStorage.getItem(`history_address_${this.props.output_wallet_type}`))[index],
    )
    ...
    fetch(
      this.props.url +
        ...
        this.props.output_wallet_type +
        '/address-validator?address=' +
        encodeURIComponent(new_withdraw_address),
      {
        method: 'get',
        headers: new Headers({ Accept: 'application/json' }),
      },
    ...
    if (
      ...
      if (!this.state.withdraw_address_is_valid) {
        ...
        if (localStorage.getItem(`history_address_${this.props.output_wallet_type}`) == null) {
          ...
          localStorage.setItem(
            `history_address_${this.props.output_wallet_type}`,
            JSON.stringify(withdrawals),
          )
        } else {
          let withdrawals = JSON.parse(
            localStorage.getItem(`history_address_${this.props.output_wallet_type}`),
          )
  ...
  onDropDownList() {
    if (
      JSON.parse(localStorage.getItem(`history_address_${this.props.output_wallet_type}`)) != null
    ) {
      ...
      if (!this.state.withdraw_address_is_valid) {
        invalid_address_message = (
          <div className="has-error" style={{ paddingTop: 10 }}>
            <Translate content="gateway.valid_address" coin_type={this.props.output_coin_type} />
          </div>
        )
        confirmation = (
