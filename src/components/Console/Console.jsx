...
export default class Console extends Component {
  ...
  on_cmd_keydown(evt) {
    ...
    switch (evt.which) {
      ...
      case keyCode.down:
        ...
        if (cmd_history.length - 1 == cmd_history_position && this.state.cmd != '') {
  ...
  run() {
    ...
    if (cmd_history_position && cmd_history[cmd_history_position - 1] == this.state.cmd)
      cmd_history.pop()
  ...
  cmd_console_result(result) {
    ...
    cmd_console.push(<div className="console_result monospace">{result_stringify}</div>)
    this.forceUpdate()
  }
  ...
  cmd_console_error(error) {
    ...
    cmd_console.push(<div className="console-error monospace has-error">{message}</div>)
    this.forceUpdate()
  }
}
