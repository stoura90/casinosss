...
class BitAssetOptions extends React.Component {
  ...
  _onFoundBackingAsset(asset) {
    if (asset) {
      if (
        asset.get('id') === '1.3.0' ||
        (asset.get('bitasset_data_id') && !asset.getIn(['bitasset', 'is_prediction_market']))
      ) {
        // eslint-disable-next-line react/prop-types
        if (asset.get('precision') !== parseInt(this.props.assetPrecision, 10)) {
          this.setState({
            error: counterpart.translate('account.user_issued_assets.error_precision', {
              // eslint-disable-next-line react/prop-types
      ...
      <div>
        ...
        <label>
          ...
          <input
            ...
            value={
              // eslint-disable-next-line react/prop-types
              bitasset_opts.force_settlement_offset_percent / assetConstants.GRAPHENE_1_PERCENT
            }
            /* eslint-disable-next-line react/prop-types */
        ...
        <label>
          ...
          <input
            ...
            value={
              // eslint-disable-next-line react/prop-types
              bitasset_opts.maximum_force_settlement_volume / assetConstants.GRAPHENE_1_PERCENT
            }
            /* eslint-disable-next-line react/prop-types */
...
class AccountAssetCreate extends React.Component {
  ...
  _onCoreRateChange(type, e) {
    ...
    if (type === 'quote') {
      ...
      amount =
        e.amount == ''
          ? '0'
          : utils.limitByPrecision(e.amount.replace(/,/g, ''), this.props.core.get('precision'))
  ...
  render() {
    ...
    if (symbolLength === 3) {
      createFee = (
        <FormattedAsset
          amount={utils.estimateFee('asset_create', ['symbol3'], globalObject)}
          asset={'1.3.0'}
        />
      )
      ...
      createFee = (
        <FormattedAsset
          amount={utils.estimateFee('asset_create', ['symbol4'], globalObject)}
          asset={'1.3.0'}
        />
      )
    } else if (symbolLength > 4) {
      createFee = (
        <FormattedAsset
          amount={utils.estimateFee('asset_create', ['long_symbol'], globalObject)}
          asset={'1.3.0'}
        />
      )
    }
