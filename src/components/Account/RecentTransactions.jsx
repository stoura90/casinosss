...
class RecentTransactions extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    ...
    if (this.props.customFilter) {
      if (
        // eslint-disable-next-line react/prop-types
        !Utils.are_equal_shallow(this.props.customFilter.fields, nextProps.customFilter.fields) ||
        // eslint-disable-next-line react/prop-types
    ...
    if (this.props.maxHeight !== nextProps.maxHeight) return true
    if (nextState.limit !== this.state.limit || nextState.csvExport !== this.state.csvExport)
      return true
  ...
  componentDidUpdate() {
    if (this.state.csvExport) {
      ...
      saveAs(
        blob,
        'btshist-' + today.getFullYear() + '-' + today.getMonth() + '-' + today.getDate() + '.csv',
      )
    }
  ...
  render() {
    ...
    let current_account_id =
      accountsList.length === 1 && accountsList[0] ? accountsList[0].get('id') : null
    let history = this._getHistory(
      accountsList,
      this.props.showFilters && this.state.filter !== 'all' ? this.state.filter : filter,
      customFilter,
    ).sort(compareOps)
        ...
        <div className="generic-bordered-box">
          ...
          <div ref="header">
            <div className="block-content-header">
              <span>
                {/* eslint-disable-next-line react/prop-types */}
                {this.props.title ? this.props.title : <Translate content="account.recent" />}
              </span>
            ...
            <table className={'table' + (compactView ? ' compact' : '')}>
              <thead>
                <tr>
                  {compactView ? null : (
                    <th className="column-hide-tiny" style={{ width: '20%' }}>
