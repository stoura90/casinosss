...
class AccountVoting extends React.Component {
  ...
  onPublish() {
    ...
    // Submit votes
    FetchChainObjects(ChainStore.getWitnessById, this.state.witnesses.toArray(), 4000)
      .then((res) => {
  ...
  validateAccount(collection, account) {
    ...
    if (collection === 'committee') {
      return FetchChainObjects(ChainStore.getCommitteeMemberById, [account.get('id')], 3000).then(
        (res) => {
  ...
  _getTotalVotes(worker) {
    return (
      parseInt(worker.get('total_votes_for'), 10) - parseInt(worker.get('total_votes_against'), 10)
    )
  }
  ...
  getBudgetObject() {
    ...
    if (budgetObject) {
      ...
      let currentID =
        idIndex + Math.floor((now - new Date(timestamp + '+00:00').getTime()) / 1000 / 60 / 60) - 1
  ...
  render() {
    ...
    let workerBudget = globalObject
      ? parseInt(globalObject.getIn(['parameters', 'worker_budget_per_day']), 10)
      : 0
    ...
    let remainingBudget = globalObject
      ? parseInt(globalObject.getIn(['parameters', 'worker_budget_per_day']), 10)
      : 0
    ...
    let workers = workerArray
      .filter((a) => {
        ...
        return new Date(a.get('work_end_date')) > now && new Date(a.get('work_begin_date')) <= now
      })
      .sort((a, b) => {
    ...
    let newWorkers = workerArray
      .filter((a) => {
        ...
        return new Date(a.get('work_begin_date')) >= now
      })
      .sort((a, b) => {
        return this._getTotalVotes(b) - this._getTotalVotes(a)
      })
    ...
    let unVotedActiveWitnesses = globalObject
      .get('active_witnesses')
      .map((a) => {
    ...
    let unVotedActiveCMs = globalObject
    ...
    return (
      <div className="grid-content">
        ...
        <div className="content-block">
          <button
            className={cnames(publish_buttons_class, { success: this.isChanged() })}
          ...
          <button
            className={'outline ' + publish_buttons_class}
          ...
          <Tab title="explorer.witnesses.title">
            <div className={cnames('content-block', { disabled: proxy_is_set })}>
              ...
              <AccountsList
                ...
                title={counterpart.translate('account.votes.w_approved_by', {
                  // eslint-disable-next-line react/prop-types
              ...
              {unVotedActiveWitnesses.size ? (
                <AccountsList
                  ...
                  withSelector={false}
                  action="add"
                  title={counterpart.translate('account.votes.w_not_approved_by', {
                    // eslint-disable-next-line react/prop-types
                    account: this.props.account.get('name'),
                  })}
                />
          ...
          <Tab title="explorer.committee_members.title">
            <div className={cnames('content-block', { disabled: proxy_is_set })}>
              ...
              <AccountsList
                ...
                title={counterpart.translate('account.votes.cm_approved_by', {
                  // eslint-disable-next-line react/prop-types
                  account: this.props.account.get('name'),
                })}
              />
              {unVotedActiveCMs.size ? (
                <AccountsList
                  ...
                  withSelector={false}
                  action="add"
                  title={counterpart.translate('account.votes.cm_not_approved_by', {
                    // eslint-disable-next-line react/prop-types
                    account: this.props.account.get('name'),
                  })}
                />
                ...
                <tbody>
                  <tr>
                    ...
                    <td style={{ paddingLeft: 20, textAlign: 'right' }}>
                      &nbsp;
                      {globalObject ? (
                        <FormattedAsset amount={totalBudget} asset="1.3.0" decimalOffset={5} />
                      ) : null}
                      <span>
                        &nbsp;(
                        {globalObject ? (
                  ...
                  <tr>
                    ...
                    <td style={{ paddingLeft: 20, textAlign: 'right' }}>
                      {' '}
                      {globalObject ? (
                        <FormattedAsset amount={unusedBudget} asset="1.3.0" decimalOffset={5} />
                      ) : null}
                    </td>
                  </tr>
                </tbody>
              ...
              <table className="table">
                <thead>
                  <tr>
                    ...
                    <th className="hide-column-small">
                      <Translate content="account.votes.creator" />
                    </th>
                    <th className="hide-column-small">
                    ...
                    <th className="hide-column-small">
                      ...
                      <div style={{ paddingTop: 5, fontSize: '0.8rem' }}>
                        (<Translate content="account.votes.daily" />)
                      </div>
                    </th>
                    <th className="hide-column-large">
                      <div>
                        <Translate content="account.votes.unclaimed" />
                      </div>
                      <div style={{ paddingTop: 5, fontSize: '0.8rem' }}>
                        (<Translate content="account.votes.recycled" />)
                      </div>
                    </th>
                    <th className="hide-column-small">
                ...
                {newWorkers.length ? (
                  <tbody>
                    <tr>
                      <td colSpan="5">
                        <Translate component="h4" content="account.votes.new" />
                      </td>
                    </tr>
                    {newWorkers}
                    <tr>
                      <td colSpan="5">
                        <Translate component="h4" content="account.votes.active" />
                      </td>
                    </tr>
                  </tbody>
                ) : null}
                  ...
                  <tr>
                    <td colSpan="3">
                      <div className="inline-block">
                        <Translate component="h4" content="account.votes.expired" />
                      </div>
                      <span>
                        &nbsp;&nbsp;
