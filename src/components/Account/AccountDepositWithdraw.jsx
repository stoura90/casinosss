...
class AccountDepositWithdraw extends React.Component {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      ...
      !utils.are_equal_shallow(
        nextState.blockTradesBackedCoins,
        this.state.blockTradesBackedCoins,
      ) ||
  ...
  UNSAFE_componentWillMount() {
    ...
    fetch('https://blocktrades.us/api/v2/coins')
      .then((reply) =>
    ...
    fetch('https://blocktrades.us/ol/api/v2/coins')
      .then((reply) =>
        reply.json().then((result) => {
          this.setState({
  ...
  render() {
    ...
    let {
      ...
      btService,
      metaService,
      depositWithdrawDefaultActiveTab,
      services,
      activeService,
    } = this.state

    let blockTradesGatewayCoins = this.state.blockTradesBackedCoins
      .filter((coin) => {
        if (coin.backingCoinType === 'muse') {
          // it is not filterring, should be MUSE
          return false
        }
          ...
          <div className="grid-content no-padding" style={{ paddingTop: 15 }}>
              ...
              <div>
                <div className="content-block">
                  <div className="float-right">
                    <a href="https://blocktrades.us" target="__blank">
                      <Translate content="gateway.website" />
                    </a>
                  </div>
                  <div className="button-group">
                    <div
                      onClick={this.toggleBTService.bind(this, 'bridge')}
                      className={cnames('button', btService === 'bridge' ? 'active' : 'outline')}
                    >
                      <Translate content="gateway.bridge" />
                    </div>
                    <div
                      onClick={this.toggleBTService.bind(this, 'gateway')}
                      className={cnames('button', btService === 'gateway' ? 'active' : 'outline')}
                    >
            ...
            {activeService === services.indexOf('Openledger (OPEN.X)') ? (
              <div>
                <div className="content-block">
                  <div className="float-right">
                    <a href="https://www.ccedk.com/" target="__blank">
                      <Translate content="gateway.website" />
                    </a>
                  </div>
                  <div className="button-group" style={{ marginBottom: 0 }}>
                    <div
                      ...
                      className={cnames('button', olService === 'gateway' ? 'active' : 'outline')}
                    >
                      <Translate content="gateway.gateway" />
                    </div>
                    <div
                      onClick={this.toggleOLService.bind(this, 'fiat')}
                      className={cnames('button', olService === 'fiat' ? 'active' : 'outline')}
                    >
                      Fiat
                    </div>
                  </div>
            ...
            {activeService === services.indexOf('Transwiser') ? (
              <div>
                <div className="float-right">
                  <a href="https://www.transwiser.com" target="_blank" rel="noreferrer">
                    <Translate content="gateway.website" />
                  </a>
                </div>
