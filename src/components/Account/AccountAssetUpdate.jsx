...
class AccountAssetUpdate extends React.Component {
  ...
  resetState(props) {
    ...
    let coreRateQuoteAssetName = ChainStore.getAsset(core_exchange_rate.quote.asset_id).get(
      'symbol',
    )
  ...
  _onCoreRateChange(type, amount) {
    ...
    amount.amount = Utils.limitByPrecision(
      amount.amount,
      type === 'quote' ? this.props.asset.get('precision') : this.props.core.get('precision'),
    )
  ...
  render() {
    ...
    let validClaim =
      claimFeesAmount > 0 &&
      Utils.get_asset_precision(asset.get('precision')) * claimFeesAmount <= unclaimedBalance
            ...
            <Tab title="explorer.asset.fee_pool.title">
              <div className="small-12 large-8 grid-content">
                ...
                <div style={{ paddingTop: '0.5rem' }}>
                  <hr />
                  <button
                    className={classnames('button', { disabled: fundPoolAmount <= 0 })}
