...
class AccountVotingProxy extends React.Component {
  ...
  onProxyAccountChange(new_proxy_account) {
    ...
    else this.props.onProxyAccountChanged(new_proxy_account, this.state.current_proxy_input)
  ...
  render() {
    ...
    if (
      ...
      this.props.currentAccount.get('id') === this.state.new_proxy_account.get('id')
    ) {
        ...
        if (account) {
          return (
            <tr key={account.get('id')}>
              ...
              <td>
                <LinkToAccountById account={account.get('id')} subpage="voting" />
              </td>
              <td className="text-right">
                <button
                  className="button"
                  onClick={this.onProxyChange.bind(this, account.get('name'))}
                >
                  Set
                </button>
              </td>
    ...
    return (
      <div className="content-block" style={{ maxWidth: '600px' }}>
        ...
        <AccountSelector
          ...
          ref="proxy_selector"
          tabIndex={1}
          onAction={this._onNavigate.bind(
            this,
            `/account/${this.state.current_proxy_input}/voting/`,
          )}
