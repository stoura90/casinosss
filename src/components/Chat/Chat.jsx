...
class Chat extends React.Component {
  ...
  _broadCastPeers() {
    let peersArray = this.connections
      .map((conn, peer) => {
        return peer
      })
      .toArray()
  ...
  _handleMessage(data) {
    ...
    if ('history' in data && data.history.length) {
      ...
      data.history
        .filter((a) => {
          return a.user !== 'Welcome to Bitshares' && a.user !== 'Welcome to Openledger'
        })
        .forEach((msg) => {
  ...
  sendHistory(c) {
    c.send({
      history: this.state.messages.filter((msg) => {
        return msg.user !== 'SYSTEM' && msg.user !== 'Welcome to Bitshares'
      }),
    })
  }
  ...
  onConnection(c) {
    ...
    setTimeout(() => {
      c.send({
        id: this._myID,
        historyCount: this.state.messages.reduce((value, msg) => {
          return value + (msg.user !== 'SYSTEM' ? 1 : 0)
        }, 0),
      })
    }, 200)
    this.forceUpdate()
  ...
  render() {
    ...
    accountOptions.push(
      <option key="default" value={this.state.anonName}>
        {this.state.anonName}
      </option>,
    )
    ...
    let settings = (
      <div style={{ padding: 10 }}>
        ...
        <div className="settings-title">
          <Translate content="chat.user" />:{' '}
        </div>
        <select value={userName} className="form-control" onChange={this.onPickAccount.bind(this)}>
          {accountOptions}
        </select>
        {/* Color */}
        <div className="settings-title">
        ...
        <div style={chatStyle} className={'chatbox'}>
          <div className={'grid-block main-content vertical ' + (docked ? 'docked' : 'flyout')}>
            <div className="chatbox-title grid-block shrink">
              ...
              <span>
                &nbsp;- <Translate content="chat.users" count={this.connections.size + 1} />
              </span>
              <div className="chatbox-pin" onClick={this._onToggleDock.bind(this)}>
                {docked ? (
                  <Icon className="icon-14px rotate" name="thumb-tack" />
                ) : (
            ...
            {loading ? (
              ...
              <div className="grid-block vertical chatbox">
                <div style={{ padding: 20 }}>
                  ...
                  <div style={{ paddingTop: 30 }}>
                    <div onClick={this._resetServer.bind(this)} className="button">
                      <Translate content="chat.reconnect" />
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div
                className="grid-block vertical no-overflow chatbox-content"
                ...
                <div>
                  ...
                  >
                    <input
                      ...
                      /* eslint-disable-next-line react/prop-types,react/no-string-refs */
                      ref="input"
                      type="text"
                    />
                  </form>
